...
| `PROIMAGE_BASE_URL` | `https://us.prorisehub.com` | No | Relay base URL. Trailing slashes are automatically stripped. |
...
- **Capability probing script**: Re-test your active key and channel group capabilities at any time:
  ```bash
...
node e2e.mjs valid          # Rejection tests: invalid ratio, invalid quality, missing file
...
- Never commit API keys or sensitive endpoints to version control.
- Keep `.env` files gitignored. Pass keys strictly through the MCP client's execution environment.
- Generated images are stored on your local disk; verify that `PROIMAGE_SAVE_DIR` permissions align with your security requirements.

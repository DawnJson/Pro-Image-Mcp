...
server.registerTool(
  ...
  async () => {
    ...
    const lines = [
      ...
      // Only the tail, because this text lands in the model's context and in client logs.
      `  api_key:       configured (ends ${cfg.apiKey.slice(-4)})`,
      ...
      "  - Which aspect ratios are accepted is PER-MODEL, not global: z-image takes only 1:1, 3:4, 4:3, 9:16",
      "    and 16:9 (1536x1024 fails with HTTP 500), while gpt-image-2 also takes 3:2 and 21:9. Ratios beyond",
      "    3:1 are refused by every model. Sizes are checked locally before a request is billed.",
